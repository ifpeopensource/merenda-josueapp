...
function NotAllowed({ isOpen, closeModal, studentData }) {
  if (!studentData) return null;

  const elapsedTime = getElapsedTime(studentData.servedAt);

  return (
    <Dialog open={isOpen} onClose={closeModal} className="fixed z-50 inset-0">
      <div className="fixed inset-0 bg-black/60" aria-hidden="true" />
      <Dialog.Panel className="fixed py-6 px-4 flex flex-col items-center justify-center top-1/2 left-1/2 m-4 w-[calc(100%-2rem)] max-w-5xl max-h-[calc(100%-2rem)] translate-x-[calc(-50%-1rem)] translate-y-[calc(-50%-1rem)] bg-red-500 rounded-lg">
        <div className="flex flex-col items-center justify-center gap-6">
          <img
            className="w-32 h-32 object-cover rounded-full text-center text-neutral-50"
            src={studentData.pictureUrl}
        ...
        <p className="font-bold mt-2 text-neutral-50 text-center text-xl">
          Este estudante já consumiu a merenda nesta sessão há:
          <br />
